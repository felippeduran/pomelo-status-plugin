...
Event.prototype.bind_session = function(session) {
  if(!session.uid) {
    ...
    if(!!err) {
      logger.error('statusService add user failed: [%s] [%s], err: %j', session.uid, session.frontendId, err.stack || err);
      return;
    }
  });
};
...
Event.prototype.close_session = function(session) {
  ...
  this.statusService.leave(session.uid, session.frontendId, function(err) {
    if(!!err) {
      logger.error('failed to kick user in statusService: [%s] [%s], err: %j', session.uid, session.frontendId, err.stack || err);
      return;
    }
  });
};